...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    if (m === "register" && mode !== "register") setMode("register");
    if (m !== "register" && mode !== "login") setMode("login");
    ...
    <div className="min-h-screen auth-vignette flex" data-testid="auth-page">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] p-12 relative overflow-hidden border-r border-white/5">
        ...
        <div className="relative z-10 space-y-6">
          <h1 className="font-display text-5xl leading-[1.05] tracking-tight text-white">
            {isRegister
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-md">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="ml-auto flex gap-1 text-xs">
              <button
                data-testid="lang-id"
                onClick={() => setLang("id")}
                className={`px-2.5 py-1 rounded-full ${lang === "id" ? "bg-white/10 text-white" : "text-[#71717A]"}`}
              >ID</button>
              <button
                data-testid="lang-en"
                onClick={() => setLang("en")}
                className={`px-2.5 py-1 rounded-full ${lang === "en" ? "bg-white/10 text-white" : "text-[#71717A]"}`}
              >EN</button>
            </div>
          </div>

          <h2 className="font-display text-3xl mb-2">{isRegister ? t("auth.create_family") : t("auth.welcome_back")}</h2>
          <p className="text-[#A1A1AA] text-sm mb-8">{isRegister ? t("auth.subtitle_register") : t("auth.subtitle_login")}</p>
          ...
          <form onSubmit={submit} className="space-y-4" data-testid="auth-form">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="email" className="text-[#A1A1AA]">{t("auth.email")}</Label>
              <Input id="email" type="email" data-testid="auth-email" value={form.email} onChange={onChange("email")} required
                className="bg-white/[0.03] border-white/10 h-11" />
            </div>
            <div className="space-y-1.5">
            ...
            >
              {submitting ? "…" : (isRegister ? t("auth.register") : t("auth.login"))}
            </Button>
          </form>
